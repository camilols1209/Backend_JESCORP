...
app.get('/', (req,res) =>{
    ...
    con.query('SELECT * FROM tablee order by ID desc limit 1', function (err, results) {
        if (err){
            return res.send(err)
        }else{
            return res.send({
...
server.on('message', (msg,rinfo) =>{
    ...
    var con = dbConnections();
    var sql = "INSERT INTO `tablee` (`Fecha`,`Latitud`, `Longitud`) VALUES ?;";
...
const bb ='192.168.1.3';
server.bind(50001,bb);
